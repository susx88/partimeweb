...
{
  $query = mysqli_query($mysqli, "SELECT *, CONCAT (PrimerNombre,' ', ApellidoPaterno) as name_user FROM postulantes WHERE RutPostulante='$_SESSION[id_user]'")
                                or die('error: '.mysqli_error($mysqli));
...
<li class="dropdown user user-menu">
  ...
  <ul class="dropdown-menu">
    ...
    <li class="user-footer">
      <div class="pull-left">
        <?php if ($_SESSION['permisos_acceso'] != "Postulante") { ?>
        <a style="width:80px" href="?module=profile" class="btn btn-default btn-flat">Perfil</a>
        <?php } else { ?>
        <a style="width:80px" href="?module=profile_post" class="btn btn-default btn-flat">Perfil</a>
