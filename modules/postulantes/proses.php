...
if (empty($_SESSION['username']) && empty($_SESSION['password'])){
            ...
            $nacionalidad  = mysqli_real_escape_string($mysqli, trim($_POST['nacionalidad']));
              ...
              $query = mysqli_query($mysqli, "UPDATE postulantes SET  
                ...
                if ($query) {
                    ...
                    echo "<script>
                    document.location.replace('../../main.php?module=postulantes&alert=2');
                    </script>";
                }         
            }
        }
    ...
    elseif ($_GET['act']=='correo') {
        if (isset($_GET['id'])) {
            $codigo = $_GET['id'];
      
            $query = mysqli_query($mysqli, "select * FROM postulantes WHERE CodPostulante='$codigo'")
                                            or die('error '.mysqli_error($mysqli));
            ...
            <p>Estimado ".$data["PrimerNombre"]." ".$data["ApellidoPaterno"]." junto con saludar, se le informa que los datos para acceder al sistema son los siguientes</p>
            <p>Link: http://partime.grupojega.cl/</p>
            <p>Usuario: ".$data["RutPostulante"]."</p>
            <p>Clave: 123456</p>

            <h4>Equipo de PartimeWeb</4>";
            $envio = EnviarCorreo ($data["Correo"],"Bienvenido a PartimeWeb",$mensaje);
            ...
            if ($envio) {
                ...
                //header("location: ../../main.php?module=postulantes&alert=3");
                 echo "<script>
                    document.location.replace('../../main.php?module=postulantes&alert=4');
                    </script>";
            }
        }
    }       
